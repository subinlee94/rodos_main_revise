...
핵심 로직에는 `인수인계(4.x)` 주석을 달아 이 문서의 기능별 항목과 연결했다. 주석은 변경된 모든 줄의 표시가 아니라 해당 로직을 둔 이유를 설명한다. 정확한 추가·삭제 범위는 GitHub의 원본 기준 커밋(`512a5e5`)과 현재 커밋의 diff로 확인한다.
...
- 응답 객체로 받은 HTTP 4xx는 오류로 반환한다. 다만 `RestTemplate` 기본 예외 처리로 4xx가 예외가 되면 현재 catch 문을 통해 로컬 fallback으로 이어질 수 있으므로 이 경로는 추가 검증이 필요하다.
